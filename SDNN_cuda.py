...
# --------------------------- Initialisation functions ------------------------#
    ...
    def gen_iter_paths(self, path_list):
        ...
        paths_iter = iter([os.path.join(path_list[0], listdir(path_list[0])[i]) for i in range(labels.size)])
        for idir in range(1, len(path_list)):
            file_names = listdir(path_list[idir])
            labels = np.append(labels, (idir+1)*np.ones(len(file_names)))
            files_tmp = iter([os.path.join(path_list[idir], file_names[i]) for i in range(len(file_names))])
...
# --------------------------- STDP Learning functions ------------------------#
    ...
    def get_STDP_idxs(self, valid, H, W, D, layer_idx):
        ...
        while np.sum(np.sum(mxv)) != 0.:
            ...
            mxv[int(max(index1 - self.offsetSTDP[layer_idx], 0)):int(min(index1 + self.offsetSTDP[layer_idx], H) + 1),
                int(max(index2 - self.offsetSTDP[layer_idx], 0)):int(min(index2 + self.offsetSTDP[layer_idx], W) + 1)] = 0
